...
flex_negllh_var <- function(params, nPBT, nGSI, ohnc, t, utGSI, ohnc_gsi, pbtGSIkey,
	...
	###################################################
	############# left off here moving up with taking "keys" out to estimate all proportions
	############# there are going to be problems though, b/c group fixed at 1 should not be a zero group...
	###################################################
	...
	if(nVar > 0 && nPBT > 0){
		for(v in 1:nVar){
			...
			for(i in 1:nPBT){
				...
				llh <- llh + sum(ohnc_var[[v]][i,] * log(tempVarMat[i,]))
			}
		}
	}
